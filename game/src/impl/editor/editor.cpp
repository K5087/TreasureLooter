...
struct AssetSyncHelper {
    void operator()(AssetLoadResult<InputConfig>& payload) {
        ...
        ctx.m_input_manager->SetConfig(ctx.m_keyboard.get(),
                                       ctx.m_gamepad_manager.get(),
                                       payload.m_payload);
    }
...
void AddEntityToScene(Entity entity,
    ...
    auto relationship = ctx.m_relation_manager->Get(root_entity);
