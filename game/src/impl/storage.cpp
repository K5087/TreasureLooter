...
std::vector<char> IOStream::Read() const {
    ...
    while ((read_size = SDL_ReadIO(m_stream, result.data() + size,
                                   result.size() - size)) > 0) {
        size += read_size;
    }
    if (read_size < 0) {
...
void IOStream::Write(const char* data, std::size_t size) {
    std::size_t write_size = SDL_WriteIO(m_stream, data, size);
    if (write_size < 0) {
