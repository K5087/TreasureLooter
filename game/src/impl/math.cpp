...
Degrees& Degrees::operator=(Radians radians) {
    m_value = radians.Value() * 180.0f / std::numbers::pi;
    return *this;
}
...
Mat33 Mat33::CreateTranslation(const Vec2& v) {
    Mat33 result;
    result.Set(2, 0, v.x);
    result.Set(2, 1, v.y);
    return result;
...
Mat33 Mat33::CreateScale(const Vec2& v) {
    Mat33 result;
    result.Set(0, 0, v.x);
    result.Set(1, 1, v.y);
    return result;
...
float Mat33::Get(std::size_t x, std::size_t y) const {
    return m_data[x][y];
}

float& Mat33::Get(std::size_t x, std::size_t y) {
    return m_data[x][y];
}

void Mat33::Set(std::size_t x, std::size_t y, float value) {
    m_data[x][y] = value;
}
...
Radians& Radians::operator=(Degrees degrees) {
    m_value = degrees.Value() * std::numbers::pi / 180.0f;
    return *this;
}
