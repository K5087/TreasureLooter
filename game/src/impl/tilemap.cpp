...
const Tile& Tileset::GetTile(std::uint32_t gid) const {
    auto idx = gid - m_firstgid;
    return m_tiles[idx];
}

bool Tileset::HasTile(std::uint32_t gid) const {
...
void Tileset::parse(const tmx::Tileset& tileset) {
    ...
    for (std::uint32_t i = 0; i < tileset.getTileCount(); i++) {
...
const Tile* Tilemap::GetTile(std::uint32_t gid) const {
