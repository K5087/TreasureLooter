...
void Renderer::DrawRect(const Rect& r, const Color& color) {
    setRendererColor(color);
    Vec2 t1 = r.m_center - r.m_half_size;
    SDL_FRect rect{t1.x, t1.y, r.m_half_size.w * 2.0f, r.m_half_size.h * 2.0f};
    SDL_CALL(SDL_RenderRect(m_renderer, &rect));
}

void Renderer::FillRect(const Rect& r, const Color& color) {
    setRendererColor(color);
    Vec2 t1 = r.m_center - r.m_half_size;
    SDL_FRect rect{t1.x, t1.y, r.m_half_size.w * 2.0f, r.m_half_size.h * 2.0f};
    SDL_CALL(SDL_RenderRect(m_renderer, &rect));
...
void Renderer::setRendererColor(const Color& color) {
    SDL_CALL(SDL_SetRenderDrawColor(m_renderer, color.r * 255, color.g * 255,
